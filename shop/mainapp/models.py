...
class Product(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if img.height < min_height or img.width < min_width:
            return MinResolutionErrorException("Разрешение изображения меньше минимального!")
        if img.height > max_height or img.width > max_width:
            new_img = img.convert('RGB')
            resized_new_img = new_img.resize((800, 800), Image.ANTIALIAS)
            filestream = BytesIO()
            resized_new_img(filestream, 'JPEG', quality=90)
            filestream.seek(0)
...
class Order(models.Model):
    ...
    STATUS_CHOICES = (
        ...
        (STATUS_IN_PROGRESS, 'Заказ в обрабоке'),
